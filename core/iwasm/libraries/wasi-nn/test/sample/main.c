 ...
 */
 ...
 void parse_detection_results(float *out_tensor, uint32_t output_size) {
     // 预定义输出数据偏移量
     uint32_t offset = 0;
     ...
     for (int i = 0; i < num; i++) {
         printf("Box %d: [%f, %f, %f, %f]\n", i, boxes[i*4]*300, boxes[i*4 + 1]*300, boxes[i*4 + 2]*300, boxes[i*4 + 3]*300);
     }
     offset += num * 4;  // 移动到下一个数据位置，跳过 boxes 部分
     ...
     offset += num;  // 移动到下一个数据位置，跳过 classes 部分
     ...
     offset += num;  // 移动到下一个数据位置，跳过 scores 部分
 ...
 test_detection(execution_target target)
 {
     ...
     size_t tensor_size = 1 * 300 * 300 * 3;  // 输入形状 (1, 300, 300, 3)
